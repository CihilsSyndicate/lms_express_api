...
export const createModul = async (req: Request, res: Response) => {
  try {
    ...
    console.log(`[MODUL] Modul baru berhasil dibuat oleh Tutor ID ${tutorId}: ${newModul.id}`);
    res.status(201).json({ message: 'Modul berhasil dibuat', data: newModul });
  } catch (error) {
    console.error('[MODUL-ERROR] Gagal membuat modul:', error);
    res.status(500).json({ message: 'Internal server error saat membuat modul.' });
  }
};
...
export const getSemuaModul = async (req: Request, res: Response) => {
  try {
    ...
    res.status(200).json({ message: 'Berhasil mengambil data modul', data: moduls });
  } catch (error) {
    console.error('[MODUL-ERROR] Gagal mengambil modul:', error);
    res.status(500).json({ message: 'Internal server error saat mengambil modul.' });
  }
};
...
export const getModulById = async (req: Request, res: Response) => {
  try {
    ...
    res.status(200).json({ message: 'Berhasil mengambil data modul', data: modul });
  } catch (error) {
...
export const updateModul = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const {
      nama_modul, deskripsi, target_waktu, tingkat_kesulitan,
      is_berbayar, harga_modul, jenjang, kelas_sekolah
    } = req.body;

    const tutorId = req.user?.id;

    const modul = await prisma.modul.findUnique({ where: { id: id as string } });
    ...
    if (modul.tutor_id !== tutorId) {
      return res.status(403).json({ message: 'Akses ditolak. Anda tidak berhak mengubah modul ini.' });
    }
    ...
    console.log(`[MODUL] Modul berhasil diupdate oleh Tutor ID ${tutorId}: ${id}`);
    res.status(200).json({ message: 'Modul berhasil diupdate', data: updatedModul });
  } catch (error) {
    console.error('[MODUL-ERROR] Gagal update modul:', error);
    res.status(500).json({ message: 'Internal server error saat update modul.' });
  }
};
...
export const deleteModul = async (req: Request, res: Response) => {
  try {
    ...
    if (modul.tutor_id !== tutorId) {
      return res.status(403).json({ message: 'Akses ditolak. Anda tidak berhak menghapus modul ini.' });
    }

    await prisma.modul.delete({ where: { id: id as string } });

    console.log(`[MODUL] Modul berhasil dihapus oleh Tutor ID ${tutorId}: ${id}`);
    res.status(200).json({ message: 'Modul berhasil dihapus' });
  } catch (error) {
    console.error('[MODUL-ERROR] Gagal hapus modul:', error);
    res.status(500).json({ message: 'Internal server error saat menghapus modul.' });
  }
};
